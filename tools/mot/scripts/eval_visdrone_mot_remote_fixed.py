"""
VisDrone MOT评估脚本 - 远程设备优化版 (适合 24GB+ 显存)
...
使用方式：
    python eval_visdrone_mot_remote_fixed.py --model best.pt --root_dir VisDrone2019-MOT-val
"""
    ...
    def __init__(self, model_path, root_dir, device='auto', tracker='bytetrack.yaml'):
        ...
        if self.device == 'cuda':
            ...
            if torch.cuda.get_device_properties(0).total_memory > 16 * 1024**3:
                self.model.fuse()  # 融合层以提高性能

        # 高性能参数（针对24GB+ GPU）
        self.imgsz = 1280  # 更高的分辨率
        self.conf_threshold = 0.3
        self.iou_threshold = 0.45
        self.max_det = 1000  # 更高的检测数���

        print(f"高性能参数 - imgsz: {self.imgsz}, conf: {self.conf_threshold}, iou: {self.iou_threshold}, max_det: {self.max_det}")
        ...
        for gt in gt_boxes:
            for dt in dt_boxes:
                iou = self._bbox_iou(
                    ...
                    [dt['x'], dt['y'], dt['x'] + dt['w'], dt['y'] + dt_box['h']]
                )
                if iou > 0.5:
...
def main():
    parser = argparse.ArgumentParser(description='VisDrone-MOT 评估 (远程优化版)')
