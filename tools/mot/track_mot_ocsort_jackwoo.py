...
def track_visdrone_mot_jackwoo_ocsort(
    model_path: str | Path,
    ...
    if output_dir is None:
        ...
        base_dir = Path("runs") / "mot" / f"{model_stem}_jackwoo_ocsort"
    ...
    for seq_dir in sequence_dirs:
        ...
        if verbose:
            print(f"[track-jackwoo-ocsort] {seq_dir.name}: {len(image_paths)} frames")

        tracker = OCSortTracker(args=args, frame_rate=30)
        all_rows = []
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Track VisDrone MOT with JackWoo-style OC-SORT and evaluate with local protocol.")
    ...
    parser.add_argument("--kalman-format", default="ocsort", help="Kalman format used by JackWoo tracker.")
...
def main() -> None:
    ...
    output_dir = track_visdrone_mot_jackwoo_ocsort(
    ...
    print(f"[track-jackwoo-ocsort] results saved to {output_dir}")
