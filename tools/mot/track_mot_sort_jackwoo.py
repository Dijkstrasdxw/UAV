...
def track_visdrone_mot_jackwoo_sort(
    model_path: str | Path,
    ...
    if output_dir is None:
        ...
        base_dir = Path("runs") / "mot" / f"{model_stem}_jackwoo_sort"
    ...
    for seq_dir in sequence_dirs:
        ...
        if verbose:
            print(f"[track-jackwoo-sort] {seq_dir.name}: {len(image_paths)} frames")

        tracker = SortTracker(args=args, frame_rate=30)
        all_rows = []
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Track VisDrone MOT with JackWoo-style SORT and evaluate with local protocol.")
    ...
    parser.add_argument("--kalman-format", default="sort", help="Kalman format used by JackWoo tracker.")
...
def main() -> None:
    ...
    output_dir = track_visdrone_mot_jackwoo_sort(
    ...
    print(f"[track-jackwoo-sort] results saved to {output_dir}")
