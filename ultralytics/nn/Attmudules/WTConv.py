...
class _WTConv2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        base_out, wavelet_out = self.forward_branches(x)
        return base_out + wavelet_out
...
class WTConv(nn.Module):
    ...
    Logic:
    - A lightweight 1x1 pre-mix improves channel interaction before WTConv.
    - WTConv explicitly keeps local/base and large-RF wavelet branches separate.
    - The gate only modulates the wavelet branch instead of the whole enhanced feature.
    - Residual output preserves original P2 detail.
    """

    def __init__(self, c: int, kernel_size: int = 5, wt_levels: int = 1, gate_reduction: int = 8):
        super().__init__()
        self.pre_pw = nn.Conv2d(c, c, kernel_size=1, bias=False)
        ...
        self.post_pw = nn.Conv2d(c, c, kernel_size=1, bias=False)
        self.post_bn = nn.BatchNorm2d(c)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        identity = x
        mixed = self.pre_act(self.pre_bn(self.pre_pw(x)))
        base_out, wavelet_out = self.wtconv.forward_branches(mixed)
        gate = self.gate(mixed)
        enhanced = base_out + gate * wavelet_out
        enhanced = self.post_bn(self.post_pw(enhanced))
