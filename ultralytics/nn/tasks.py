...
def parse_model(d, ch, verbose=True):
    ...
    #娣诲姞娉ㄦ剰妯″潡鍚嶉泦鍚?
    attn_mudules=frozenset({CA,CAA,CBAM,CoordAtt,ECA,EMA,MLCA,GAM,SE,SK,MSCAAttention,SCSA,TripletAttention})
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in repeat_modules:
                ...
                n = 1
            if m in frozenset({C3k2, C3k2_SCConv}):  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
            ...
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            ...
            c2 = in_ch[0]
            args = [len(in_ch), *args]
        elif m in frozenset({BiFPNConcat2, BiFPNConcat3, BiFPNConcatDyn2, BiFPNConcatDyn3}):
            c2 = sum(ch[x] for x in f)
            args = [[ch[x] for x in f], *(args if len(args) else [1])]
        elif m in frozenset({DynamicFusion2, DynamicFusion3, CondFusion2, CondFusion3}):
            c2 = args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [[ch[x] for x in f], c2, *args[1:]]
        elif m in frozenset({AFPNConcat2, AFPNConcat3, AFPNWCGConcat2, AFPNWCGConcat3}):
            c2 = sum(ch[x] for x in f)
            args = [[ch[x] for x in f], *(args if len(args) else [1])]
        elif m in frozenset({ASFFP2, ASFFP3, ASFFP4}):
            if len(f) != 3:
                raise ValueError(f"{m.__name__} expects 3 input feature maps, got from={f}")
            args = [[ch[x] for x in f], *args]
            if m is ASFFP2:
                c2 = ch[f[0]]
            elif m is ASFFP3:
