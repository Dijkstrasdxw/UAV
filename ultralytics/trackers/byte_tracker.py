...
class STrack(BaseTrack):
    ...
    Attributes:
        shared_kalman (KalmanFilterXYWH): Shared Kalman filter used across all STrack instances for prediction.
        _tlwh (np.ndarray): Private attribute to store top-left corner coordinates and width and height of bounding box.
        kalman_filter (KalmanFilterXYWH): Instance of Kalman filter used for this particular object track.
    ...
    Methods:
        ...
        convert_coords: Convert bounding box to x-y-width-height format.
        tlwh_to_xywh: Convert tlwh bounding box to xywh format.
    ...
    def convert_coords(self, tlwh: np.ndarray) -> np.ndarray:
        """Convert a bounding box's top-left-width-height format to its x-y-width-height equivalent."""
    ...
    def tlwh_to_xywh(tlwh: np.ndarray) -> np.ndarray:
        """Convert bounding box from tlwh format to center-x-center-y-width-height (xywh) format."""
...
class BYTETracker:
    ...
    Attributes:
        ...
        kalman_filter (KalmanFilterXYWH): Kalman Filter object.
    ...
    def __init__(self, args, frame_rate: int = 30):
        ...
        self.gmc = None
        self.prev_img = None
        gmc_method = getattr(args, "gmc_method", "none")
        if gmc_method and str(gmc_method).lower() != "none":
    ...
    def update(self, results, img: np.ndarray | None = None, feats: np.ndarray | None = None) -> np.ndarray:
        ...
        if (
            getattr(self.args, "with_color_match", False)
            and self.prev_img is not None
            and img is not None
            and len(r_tracked_stracks)
            and len(detections_second)
        ):
            color_dists = matching.color_histogram_distance(r_tracked_stracks, detections_second, self.prev_img, img)
            color_dists = matching.keep_top_n(color_dists, n=int(getattr(self.args, "color_topk", 3)))
            dists = matching.fuse_iou_with_color(dists, color_dists)
    ...
    def get_kalmanfilter(self) -> KalmanFilterXYWH:
        """Return a Kalman filter object for tracking bounding boxes using KalmanFilterXYWH."""
