...
def non_max_suppression(
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        if rotated:
            ...
            if soft_nms:
                i, scores_updated = soft_nms_boxes(
                    boxes=boxes,
                    scores=scores,
                    iou_thres=soft_nms_iou,
                    sigma=soft_nms_sigma,
                    method=soft_nms_method,
                    score_thres=soft_nms_score,
                    max_det=max_det,
                )
                if i.numel() == 0:
                    continue
...
def soft_nms_boxes(
    ...
    """Soft-NMS for axis-aligned boxes.

    Args:
        boxes (torch.Tensor): Boxes in xyxy format, shape (N, 4).
        scores (torch.Tensor): Scores, shape (N,).
        iou_thres (float): IoU threshold for linear Soft-NMS.
        sigma (float): Sigma for gaussian Soft-NMS.
        method (str): "linear" or "gaussian".
        score_thres (float): Score threshold to discard boxes during Soft-NMS.
        max_det (int): Maximum number of detections to keep.

    Returns:
        (torch.Tensor, torch.Tensor): kept indices and updated scores (same shape as input scores).
    """
    if boxes.numel() == 0:
    ...
    if method not in {"linear", "gaussian"}:
        raise ValueError(f"soft_nms method must be 'linear' or 'gaussian', got {method!r}")
    ...
    while scores_work.numel() > 0:
        max_pos = scores_work.argmax()
        keep.append(idxs_work[max_pos])
        if len(keep) >= max_det:
            break

        best_box = boxes_work[max_pos].unsqueeze(0)
        boxes_work = torch.cat((boxes_work[:max_pos], boxes_work[max_pos + 1 :]), dim=0)
        scores_work = torch.cat((scores_work[:max_pos], scores_work[max_pos + 1 :]), dim=0)
        idxs_work = torch.cat((idxs_work[:max_pos], idxs_work[max_pos + 1 :]), dim=0)

        if boxes_work.numel() == 0:
            break

        ious = box_iou(best_box, boxes_work).squeeze(0)
        if method == "linear":
            decay = torch.ones_like(ious)
            mask = ious > iou_thres
            decay[mask] = 1.0 - ious[mask]
        else:
            decay = torch.exp(-(ious * ious) / max(sigma, 1e-6))

        scores_work = scores_work * decay
        scores_updated[idxs_work] = scores_work

        keep_mask = scores_work >= score_thres
        boxes_work = boxes_work[keep_mask]
        scores_work = scores_work[keep_mask]
        idxs_work = idxs_work[keep_mask]

    if keep:
        keep = torch.stack(keep)
    else:
        keep = torch.empty((0,), dtype=torch.int64, device=boxes.device)
    return keep, scores_updated
